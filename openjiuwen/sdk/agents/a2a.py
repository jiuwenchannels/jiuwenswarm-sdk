...
from openjiuwen.sdk.core.errors import AgentError, ConnectionError, SdkError, StreamError
...
class RemoteAgent:
    ...
    async def run(self, prompt: str, *, task_id: str | None = None) -> A2AResult:
        ...
        Raises:
            :class:`~openjiuwen.sdk.core.errors.ConnectionError`: service unreachable.
    ...
    async def stream(self, prompt: str, *, task_id: str | None = None) -> AsyncIterator[str]:
        ...
        Raises:
            :class:`~openjiuwen.sdk.core.errors.ConnectionError`: service unreachable.
            :class:`~openjiuwen.sdk.core.errors.StreamError`:   streaming broke.
        """
        client = await self._get_client()
        task_id = task_id or f"task-{uuid.uuid4().hex[:8]}"
        inputs: dict[str, Any] = {"query": prompt, "task_id": task_id}
