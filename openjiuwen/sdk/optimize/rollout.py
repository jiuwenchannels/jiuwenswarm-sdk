...
Quick start::
    ...
    from openjiuwen.sdk.optimize.rollout import MultiRolloutConfig, MultiRolloutExecutor
...
if TYPE_CHECKING:
    ...
    from openjiuwen.sdk.optimize.eval import EvalCase, Metric
    ...
    async def best_of(
        ...
        *,
        ...
        Args:
            ...
            metric:  Any :class:`~openjiuwen.sdk.optimize.eval.Metric` instance.

        Returns:
            The :class:`RolloutResult` with the highest score.

        Raises:
            :class:`~openjiuwen.sdk.core.errors.SdkError`: If *results* is empty.
        """
        if not results:
            raise SdkError("best_of received an empty results list")

        from openjiuwen.sdk.optimize.eval import EvalCase
